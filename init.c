...
#define SYS_PORT_E_LAT 0x0    // Настройка регистра LAT для порта E
 ...
 * - Устанавливает бит разрешения прерывания таймера T4 в соответствии с переданным зн  ачением.
...
void tmr_6_init()
{
    ...
    PR6 = 62206;       // Установка значения предварительной загрузки для сравнения с таймером T6
    // PR6 = 7776;            // Установка значения предварительной загрузки для сравнения с таймером T6
    IEC0bits.T6IE = 1;      // Установка бита разрешения прерывания таймера T6 в соответствии с переданным значением
    IPC7bits.T6IP = 0b10;   // Установка приоритета прерывания таймера T6 в 2
    T6CONbits.TCKPS = 0b10; // Установка предделителя таймера T6 на значение 1:64
    T6CONbits.TON = 1;      // Включение таймера T6 в соответствии с переданным значением
}
...
/**
 * @brief Инициализация модуля сравнения вывода OC3.
 *
 * Эта функция инициализирует модуль сравнения вывода OC3. Сначала модуль сравнения OC3
 * выключается, затем сбрасываются настройки таймера 4. Устанавливаются значения основного и
 * вторичного регистров сравнения. Затем настраивается режим широтно-импульсной модуляции (ШИМ)
 * без использования пина Fault. Устанавливается предделитель таймера 4 на значение 1:1, период
 * таймера 4 устанавливается в соответствии с частотой ШИМ. Флаг прерывания таймера 4 сбрасывается,
 * разрешаются прерывания от таймера 4, устанавливается приоритет прерывания таймера 4.
 *
 * @param void
 * @return void
 */
...
/**
 * @brief Инициализация USART5.
 *
 * Настройка режима передачи, формата кадра, скорости передачи данных, прерываний передачи и приема.
 * USART5 используется для передачи и приема данных по UART.
 *
 * @param void
 * @return void
 */
...
/**
 * @brief Инициализация USART4.
 *
 * Настройка режима передачи, формата кадра, скорости передачи данных, прерываний передачи и приема.
 * USART4 используется для передачи и приема данных по UART.
 *
 * @param void
 * @return void
 */
...
/**
 * @brief Инициализация USART3.
 *
 * Настройка режима передачи, формата кадра, скорости передачи данных, прерываний передачи и приема.
 * USART3 используется для передачи и приема данных по UART.
 *
 * @param void
 * @return void
 */
 ...
 *
 * Настройка режима передачи, формата кадра, скорости передачи данных, прерываний передачи и приема.
 * USART2 используется для передачи и приема данных по UART.
 *
 * @param void
 * @return void
 */
 ...
 *
 * Настройка режима передачи, формата кадра, скорости передачи данных, прерываний передачи и приема.
 * USART1 используется для передачи и приема данных по UART.
 *
 * @param void
 * @return void
 */
...
/**
 * @brief Инициализация USART1 с заданной скоростью передачи данных.
 *
 * Настройка режима передачи, формата кадра, скорости передачи данных, прерываний передачи и приема для USART1.
 *
 * @param speed Скорость передачи данных (в бодах).
 * @return void
 */
 ...
 * @brief Инициализация USART1 с заданной скоростью передачи данных.
 *
 * Настройка режима передачи, формата кадра, скорости передачи данных, прерываний передачи и приема для USART2.
 *
 * @param speed Скорость передачи данных (в бодах).
 * @return void
 */
void UART2_init(unsigned int speed)
{
    ...
    case 1:
    {
        U2BRG = PBCLK2_ / (9600 * 16) - 1;          // Установка скорости передачи 9600 бод
        frame_delay = Fcy / 8 / 9600 * rx_timeout1; // Рассчет задержки между фреймами
        break;
    }
    ...
    case 2:
    {
        U2BRG = PBCLK2_ / (19200 * 16) - 1;          // Установка скорости передачи 19200 бод
        frame_delay = Fcy / 8 / 19200 * rx_timeout1; // Рассчет задержки между фреймами
        break;
    }
    ...
    case 3:
    {
        U2BRG = PBCLK2_ / (38400 * 16) - 1;          // Установка скорости передачи 38400 бод
        frame_delay = Fcy / 8 / 38400 * rx_timeout1; // Рассчет задержки между фреймами
        break;
    }
    ...
    case 4:
    {
        U2BRG = PBCLK2_ / (57600 * 16) - 1;          // Установка скорости передачи 57600 бод
        frame_delay = Fcy / 8 / 57600 * rx_timeout1; // Рассчет задержки между фреймами
        break;
    }
    ...
    case 5:
    {
        U2BRG = PBCLK2_ / (115200 * 16) - 1;          // Установка скорости передачи 115200 бод
        frame_delay = Fcy / 8 / 115200 * rx_timeout1; // Рассчет задержки между фреймами
        break;
    }
...
/**
 * @brief Инициализация USART4 с заданной скоростью передачи данных.
 *
 * Настройка режима передачи, формата кадра, скорости передачи данных, прерываний передачи и приема для USART4.
 *
 * @param speed Скорость передачи данных (в бодах).
 * @return void
 */
void UART4_init(unsigned int speed)
{
    ...
    case 1:
    {
        U4BRG = PBCLK2_ / (9600 * 16) - 1;          // Установка скорости передачи 9600 бод
        frame_delay = Fcy / 8 / 9600 * rx_timeout1; // Рассчет задержки между фреймами
        break;
    }
    ...
    case 2:
    {
        U4BRG = PBCLK2_ / (19200 * 16) - 1;          // Установка скорости передачи 19200 бод
        frame_delay = Fcy / 8 / 19200 * rx_timeout1; // Рассчет задержки между фреймами
        break;
    }
    ...
    case 3:
    {
        U4BRG = PBCLK2_ / (38400 * 16) - 1;          // Установка скорости передачи 38400 бод
        frame_delay = Fcy / 8 / 38400 * rx_timeout1; // Рассчет задержки между фреймами
        break;
    }
    ...
    case 4:
    {
        U4BRG = PBCLK2_ / (57600 * 16) - 1;          // Установка скорости передачи 57600 бод
        frame_delay = Fcy / 8 / 57600 * rx_timeout1; // Рассчет задержки между фреймами
        break;
    }
    ...
    case 5:
    {
        U4BRG = PBCLK2_ / (115200 * 16) - 1;          // Установка скорости передачи 115200 бод
        frame_delay = Fcy / 8 / 115200 * rx_timeout1; // Рассчет задержки между фреймами
        break;
    }
...
/**
 * @brief Инициализация USART5 с заданной скоростью передачи данных.
 *
 * Настройка режима передачи, формата кадра, скорости передачи данных, прерываний передачи и приема для USART5.
 *
 * @param speed Скорость передачи данных (в бодах).
 * @return void
 */
void UART5_init(unsigned int speed)
{
    ...
    case 1:
    {
        U5BRG = PBCLK2_ / (9600 * 16) - 1;          // Установка скорости передачи 9600 бод
        frame_delay = Fcy / 8 / 9600 * rx_timeout1; // Рассчет задержки между фреймами
        break;
    }
    ...
    case 2:
    {
        U5BRG = PBCLK2_ / (19200 * 16) - 1;          // Установка скорости передачи 19200 бод
        frame_delay = Fcy / 8 / 19200 * rx_timeout1; // Рассчет задержки между фреймами
        break;
    }
    ...
    case 3:
    {
        U5BRG = PBCLK2_ / (38400 * 16) - 1;          // Установка скорости передачи 38400 бод
        frame_delay = Fcy / 8 / 38400 * rx_timeout1; // Рассчет задержки между фреймами
        break;
    }
    ...
    case 4:
    {
        U5BRG = PBCLK2_ / (57600 * 16) - 1;          // Установка скорости передачи 57600 бод
        frame_delay = Fcy / 8 / 57600 * rx_timeout1; // Рассчет задержки между фреймами
        break;
    }
    ...
    case 5:
    {
        U5BRG = PBCLK2_ / (115200 * 16) - 1;          // Установка скорости передачи 115200 бод
        frame_delay = Fcy / 8 / 115200 * rx_timeout1; // Рассчет задержки между фреймами
        break;
    }
    default:
...
/**
 * @brief Инициализация модуля SPI5.
 *
 * Настройка режима работы, скорости передачи данных и других параметров для модуля SPI5.
 * Расчет скорости передачи данных и BRG (Baud Rate Generator) осуществляется на основе заданной желаемой скорости передачи.
 *
 * @return void
 */
void spi5_init(void)
...
void InitializeSystem(void)
{
    ...
    SYSKEY = 0x556699AA;
    // Modify PB3DIV
    PB2DIVbits.PBDIV = 3; // делим частоту системной шины на 4 для переферийно шины 2
    CFGCONbits.OCACLK = 1;
    PB3DIVbits.PBDIV = 3; // делим частоту системной шины на 4 для переферийно шины 2
    // Lock sequence
    SYSKEY = 0x33333333;
    ...
    tmr_3_init(100, 0, 0);   // Инициализация таймера 2
    tmr_4_init(100, 0, 0);   // Инициализация таймера 2
    tmr_5_init(100, 0, 0);   // Инициализация таймера 5
    tmr_6_init();            // Инициализация таймера 6
    ...
    /* Включить периферию */
    T1CONbits.ON = 1;  // Включение таймера 1
    T2CONbits.ON = 1;  // Включение таймера 2
    T3CONbits.ON = 1;  // Включение таймера 3
    T4CONbits.ON = 1;  // Включение таймера 4
    T5CONbits.ON = 1;  // Включение таймера 5
    T6CONbits.ON = 1;  // Включение таймера 6
