...
float motor_I_routine(float motor_InputErr, float motor_Ti, float motor_Ki)
{
    ...
    if (motor_Ki)
    {
        motor_coeff = motor_Ti / motor_Ki;
    }
    motor_I_Result = motor_I_Result + (motor_coeff * motor_InputErr);
    if (motor_I_Result >= 100.0)
    {
        motor_I_Result = 100.0;
    }
    if (motor_I_Result <= -100.0)
    {
        motor_I_Result = -100.0;
...
float motor_regulator(float motor_Kp, float motor_Ki, float motor_Kd)
{
    ...
    if ((motor_Clearance_Error < MB_conf.PID_deadband) || (motor_Clearance_Error > (MB_conf.PID_deadband * (-1.0))))
    {
        motor_P_Result = motor_P_routine(motor_Clearance_Error, motor_Kp);
    }
    ...
    if (motor_P_Result > MB_conf.KP_res_limit)
    {
        motor_P_Result = MB_conf.KP_res_limit;
    }
    if (motor_P_Result < ((-1.0) * MB_conf.KP_res_limit))
    {
        motor_P_Result = (-1.0) * MB_conf.KP_res_limit;
    }
