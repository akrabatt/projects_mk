...
void clr_charge_err (void) {    close_1 = 0;    close_2 = 0;    }

void ignit_start (unsigned short ch_num)    {
    
    if(CTRL1 == 1) {close_1++;}    else {close_1 = 0;}
    if(CTRL2 == 1) {close_2++;}    else {close_2 = 0;}

    
    if (close_1 >= 10)    {
        close_2 = 0;   close_1 = 0; charge_fail = 1;
        Modbus.AO_cyl_num = 1; Modbus_sw.AO_cyl_num = swapshort (Modbus.AO_cyl_num);
        return;  }
    
    if (close_2 >= 10)    {
        close_2 = 0;   close_1 = 0; charge_fail = 1;
        Modbus.AO_cyl_num = 2; Modbus_sw.AO_cyl_num = swapshort (Modbus.AO_cyl_num);
        return;  }

    switch (ch_num)
        {
        case 1: {ON_IGN_1; ign_IC8 = 1; pulse_started1 = 1; ign_timer8 = TMR2; break;}
        case 2: {ON_IGN_2; ign_IC7 = 1; pulse_started2 = 1; ign_timer7 = TMR2; break;}
        case 3: {ON_IGN_3; ign_IC8 = 1; pulse_started1 = 1; ign_timer8 = TMR2; break;}
        case 4: {ON_IGN_4; ign_IC7 = 1; pulse_started2 = 1; ign_timer7 = TMR2; break;}
        case 5: {ON_IGN_5; ign_IC8 = 1; pulse_started1 = 1; ign_timer8 = TMR2; break;}
        case 6: {ON_IGN_6; ign_IC7 = 1; pulse_started2 = 1; ign_timer7 = TMR2; break;}
        case 7: {ON_IGN_7; ign_IC8 = 1; pulse_started1 = 1; ign_timer8 = TMR2; break;}
        case 8: {ON_IGN_8; ign_IC7 = 1; pulse_started2 = 1; ign_timer7 = TMR2; break;}
        case 9: {ON_IGN_9; ign_IC8 = 1; pulse_started1 = 1; ign_timer8 = TMR2; break;}
        case 10: {ON_IGN_10; ign_IC7 = 1; pulse_started2 = 1; ign_timer7 = TMR2; break;}
        }
//    pulse_started = 1;
    ign_wait = 3;
...
void Timer6Interrupt(void)    
    { ignit_start (ign_six); active_cylinder = ign_six - 1; T6CONbits.TON=0; IFS0bits.T6IF=0; ign_stat_even = 2; }

void tmr_6_init (unsigned short T6_delay, unsigned short TMR6_IE, unsigned short TMR6_ON, unsigned short chan_num)
    { ign_six = chan_num; T6CONbits.ON = 0; TMR6 = 0x0; PR6 = T6_delay; IEC0bits.T6IE = TMR6_IE; T6CONbits.ON = TMR6_ON; }

/*              каналы 1, 3, 5, 7, 9         */
void Timer7Interrupt(void)      
    { ignit_start (ign_seven); active_cylinder = ign_seven - 1; T7CONbits.TON=0; IFS1bits.T7IF=0; ign_stat_even = 2; }

void tmr_7_init (unsigned short T7_delay, unsigned short TMR7_IE, unsigned short TMR7_ON, unsigned short chan_num)
    { ign_seven = chan_num; T7CONbits.ON = 0; TMR7 = 0x0; PR7 = T7_delay; IEC1bits.T7IE = TMR7_IE; T7CONbits.ON = TMR7_ON; }
...
void ignit_stop (void)       // выключение зажигания и контроль качества искрообразования
{
    ...
    if (( pulse_started1 == 0 ) && ( pulse_started2 == 0 )) {return;}
    if ( ign_wait != 0 ) {ign_wait--; return;}
        /* идет зажигание по цилиндрам  1-3-5-7-9 */
        if (pulse_started1 == 1) {
            br_dir0 = dir_br_low [ign_seven-1];
            br_dir1 = dir_br_low [ign_seven-1];
            if (ign_IC8 == 1) {MB_diagn.break_low [ign_seven -1]++; br_dir1++; total_err_count1 ++;} else {br_dir1 = 0; total_err_count1 = 0;}
            if (br_dir1 > MB_conf.NUM_ERR_AO) {br_dir1 = MB_conf.NUM_ERR_AO; }
     ...
     if (pulse_started2 == 1) {
            ...
            if (ign_IC7 == 1) {MB_diagn.break_low [ign_six -1]++; br_dir1++; total_err_count2 ++;} else {br_dir1 = 0; total_err_count2 = 0;}
            if (br_dir1 > MB_conf.NUM_ERR_AO) {br_dir1 = MB_conf.NUM_ERR_AO; }
...
void ignit_distributor (void)                          // режим работы по алгоритму от давления регулирования УОЗ
{
    ...
    if (sett_diff <=0 ) {sett_diff = 1;}
    
 if (ch_1.rpm >= RPM_UOZ_MAX) { Modbus.actual_UOZ = NOM_UOZ ; }               // calculate UOZ depending of RPM 
    else {
        if (ch_1.rpm < RPM_UOZ_MIN) { Modbus.actual_UOZ = LOW_UOZ; } 
        else { Modbus.actual_UOZ = (ch_1.rpm - RPM_UOZ_MIN) * NOM_UOZ / sett_diff;  }
        }
    ...
    if ((cylinder == 2) || (cylinder == 4) || (cylinder == 6) || (cylinder == 8) || (cylinder == 10))   {   tmr_6_init (tmr_set, 1, 1, cylinder);  }
    if ((cylinder == 1) || (cylinder == 3) || (cylinder == 5) || (cylinder == 7) || (cylinder == 9))    {   tmr_7_init (tmr_set, 1, 1, cylinder);  }

    sync_counter_11 ++;     LED9 ^=1;
}

void ignit_check (unsigned short cyl_num)  {  //  ign_err_check ();                  // проверка зажигания
    if ((cyl_num == 1) || (cyl_num == 3) || (cyl_num == 5) || (cyl_num == 7) || (cyl_num == 9))    {   tmr_7_init (150, 1, 1, cyl_num);  }
    if ((cyl_num == 2) || (cyl_num == 4) || (cyl_num == 6) || (cyl_num == 8) || (cyl_num == 10))   {   tmr_6_init (150, 1, 1, cyl_num);  }
    }

unsigned short bit_set (unsigned short cyl, unsigned short candle) {
    unsigned short bit_data;
    bit_data = 1 << (cyl - 1 + candle);    return bit_data; }
...
//    oddeven = 1;
    ...
    sc_count = MB_diagn.short_low [ign_six - 1];     br_count = MB_diagn.break_high [ign_six - 1];
    sc_limit = MB_conf.discharge_sc_low;    br_limit = MB_conf.discharge_br_high;
    
    while (IC7CONbits.ICBNE)	{IC_hold = IC7BUF;	}
    if (pulse_started2 == 1){
            if (IC_hold  > ign_timer7) {IC_delta = IC_hold  - ign_timer7;}
            else {IC_delta = (0xFFFFFFFF - ign_timer7) + IC_hold  +1;}
//            discharge = (float) IC_delta / 49.7664;
            discharge = (float) IC_delta * 0.02009388;
            MB.fl_2 = discharge;
            if (discharge < sc_limit) { sc_count++; sc_dir++;  total_err_count2 ++;} else {sc_dir = 0; total_err_count2 = 0;}    //контроль на короткое замыкание низковольтой части
...
//    oddeven = 0;
    ...
    while (IC8CONbits.ICBNE)	{IC_hold = IC8BUF;	}
    if (pulse_started1 == 1){
            if (IC_hold  > ign_timer8) {IC_delta = IC_hold  - ign_timer8;}
            else {IC_delta = (0xFFFFFFFF - ign_timer8) + IC_hold  +1;}
//            discharge = (float) IC_delta / 49.7664;
            discharge = (float) IC_delta * 0.02009388;
            MB.fl_2 = discharge;
            if (discharge < sc_limit) { sc_count++; sc_dir++;  total_err_count1 ++;} else {sc_dir = 0;  total_err_count1 = 0;}    //контроль на короткое замыкание низковольтой части
            if (discharge > br_limit) { br_count++; br_dir++; } else {br_dir = 0;}                                              //контроль на обрыв высоковольтой части
            if (sc_dir > MB_conf.NUM_ERR_AO) {sc_dir = MB_conf.NUM_ERR_AO; }
            if (br_dir > MB_conf.NUM_ERR_AO) {br_dir = MB_conf.NUM_ERR_AO; }
    ...
    void IC7Int (void)  {   ign_IC7 = 0; IGN_RESET;  pulse_started2 = 0;
//    if ((ign_IC7 == 0) && (ign_IC8 == 0)) {      //        ignit_stop () ;
//        }
    }
    
    void IC8Int (void)  {   ign_IC8 = 0; IGN_RESET;  pulse_started1 = 0;
//    if ((ign_IC7 == 0) && (ign_IC8 == 0)) {     //        ignit_stop () ;
//        }    
    }
