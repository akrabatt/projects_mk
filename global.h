...
typedef struct tag_MOPS {
    ...
    struct {
        ...
        union {
            ...
            struct {
                unsigned int info[3]; // идентификационная область: тип прибора, версия прошивки, версия платы
            ...
            unsigned int read[num_zone_mops * 3 + 4]; // область чтения
        };
        ...
        union {
            ...
            struct {
                ...
                unsigned int type[num_zone_mops]; // типы зон: обычная, пожарная с ПЗ и без ПЗ
                unsigned int limit1[num_zone_mops]; // пороги обыв - норма для всех зон
                unsigned int limit2[num_zone_mops]; // пороги норма - внимание для всех зон
                unsigned int limit3[num_zone_mops]; // пороги внимание - пожар для всех зон
                unsigned int limit4[num_zone_mops]; // пороги пожар - КЗ для всех зон
