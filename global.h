...
struct tag_usartm
{
    ...
    {
        unsigned modb_mode : 1;             // 1 бит для режима MODBUS
        unsigned term_mode : 1;             // 2 бит для режима терминала
        unsigned modb_received : 1;         // 3 Флаг получения MODBUS
        unsigned modb_transmited : 1;       // 4 Флаг передачи MODBUS
        unsigned modb_receiving : 1;        // 5 Флаг приема MODBUS
        unsigned modb_transmiting : 1;      // 6 Флаг отправки MODBUS
        unsigned tx_mode : 1;               // 7 Режим передачи (USART TX mode) прерывание или ДМА
        unsigned mbm_data_rdy : 1;          // 8 Готовность данных к передаче
        unsigned master_error : 1;          // 9 Ошибка мастера
        unsigned bussy1 : 1;                // 10 Резервное поле
        unsigned bussy2 : 1;                // 11 Резервное поле
        unsigned last_byte : 1;             // 12 Флаг последнего байта
        unsigned byte_missing : 1;          // 13 Отсутствие байта
        unsigned coll_1 : 1;                // 14 Коллизия 1
        unsigned coll_2 : 1;                // 15 Коллизия 2
        unsigned coll_3 : 1;                // 16 Коллизия 3
        unsigned start16 : 1;               // 17 Начало передачи
        unsigned master_start : 1;          // 18 Начало мастера
        unsigned master_timeout_flag : 1;   // 19 флаг ошибки тайм-аута
        unsigned tm_on : 1;                 // 20 Включение таймера
        unsigned device_error : 1;          // 21 Ошибка устройства
        unsigned crc_error : 1;             // 22 Ошибка CRC
        unsigned : 10;                      // Резервные биты (23-32)
    } mb_status;
...
union tag_Modbus {
	...
	unsigned short AO_cyl_num;      //24
        ...
        unsigned short pulses_11;       //27    количество импульсов по каналу синхронизации на момент синхронизации
	unsigned short pulses_40;	//28	количество импульсов по каналу оборотов на один оборот по каналу синхронизации
        float err_rpm;                  //29
        union {
		struct {
