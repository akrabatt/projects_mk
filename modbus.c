...
void close_mb (struct tag_usart * usart)            // закрытие обработки модбас, сброс в начало
{
...
void mbs_uni (struct tag_usart * usart, unsigned char mbs_addres) 
{
	if (usart->mb_status.modb_received)	{	
		...
		if (uchCRCLo|uchCRCHi) 	{	close_mb (usart);     return;	}					// no actions
		...
		default :{
			switch (usart->in_buffer[1])	{
				case 0x03 :	{
                    ...
                    if (CONF_READ_) 	{ mbs_03 (usart, MB_sw_conf.buf, (start_reg - START_CONF_READ), num_reg); break; }      // 500 ... 680
                    if (MODBUS_READ_) 	{ mbs_03 (usart, Modbus_sw.buf, (start_reg - START_MODBUS_READ), num_reg); break; }     // 0 ... 80
                    if (MB_DIAGN_READ_) { mbs_03 (usart, MB_diagn_sw.buf, (start_reg - START_MB_DIAGN_READ), num_reg); break; }  // 200 ... 312
					answer_illegal_data_addr (usart);
					break; }			

					case 0x10 :	{
			 		if (WRITE_)         { mbs_10 (usart, MB_swap.input, (start_reg - START_WRITE), num_reg); break; } 
                    if (CONF_WRITE_) 	{ mbs_10 (usart, MB_sw_conf.buf, (start_reg - START_CONF_WRITE), num_reg); break; } 
                    if (MODBUS_WRITE_) 	{ mbs_10 (usart, Modbus_sw.buf, (start_reg - START_MODBUS_WRITE), num_reg); break; } 
					answer_illegal_data_addr (usart);					
					break;	}
...
void mbm_03 (struct tag_usart * usart, unsigned char mbm_adres, unsigned short shift_03, unsigned short quant_03, unsigned short * dest, unsigned long speed) 
{
	...
	switch (usart->mbm_status) {
		case 0:	{
			...
			switch (speed){
				case 1200:      {usart->mbm_timeout_counter = 1100; break;}
				case 2400:      {usart->mbm_timeout_counter = 1100; break;}
				case 4800:      {usart->mbm_timeout_counter = 660; break;}
				case 9600:      {usart->mbm_timeout_counter = 660; break;}
				case 19200:     {usart->mbm_timeout_counter = 660; break;}
				case 38400:     {usart->mbm_timeout_counter = 300; break;}
				case 57600:     {usart->mbm_timeout_counter = 500; break;}
				case 115200:    {usart->mbm_timeout_counter = 500; break;}
				}
