...
// для шорта
// эта функция переставляет байты младшего и старшего порядка в переданном 16-бином значении data
unsigned short swapshort(unsigned short data)
{
    // используется объединение union, чтоб преобразовать значение data в массив байтов cdata и затем поменять
    // их местами
    union
    {
