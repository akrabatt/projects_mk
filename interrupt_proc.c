...
void discret_ctrl (void)
{
    ...
    if (DINPUT3 != DI_3) {  DI3_cnt += CYCLE; 
        if (DI3_cnt >= FILT_DISCRET) { DINPUT3 = DI_3 ; DI3_cnt = 0; } 
        }
    else { DI3_cnt = 0;}    
}
...
//        mpid_control(MB_conf.KP_mpid, MB_conf.KD_mpid, MB_conf.KI_mpid);
        ...
        discret_ctrl ();
        start_ctrl = 1;
//        main_control ();                                                        // управление алгоритмом
        if (_100msec >=10) {_100msec = 0; help_strobe  ^= 1;  stop_inactive (); 

        if ((MB_conf.max_RPM <= 350) || (MB_conf.max_RPM >= 390)) {load_config ();  }          //help_load =0;
        }    //
        if (_1000msec >= 100) {_1000msec = 0;  conf_check ();}
    }
    _divider++;
}
