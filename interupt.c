...
void __ISR_AT_VECTOR (_TIMER_1_VECTOR, IPL4SRS) T1Interrupt(void)   {T1CONbits.TON=0; T1Interrupt_(&usart5); IFS0bits.T1IF = 0;}

void __ISR_AT_VECTOR (_TIMER_9_VECTOR, IPL4SRS) T9Interrupt(void)   {T9CONbits.TON=0; T9Interrupt_(&usart4); IFS1bits.T9IF = 0;}
...
void __ISR_AT_VECTOR (_TIMER_4_VECTOR, IPL4SRS) T4Interrupt(void)   {    ignit_stop () ;   ADCCON3bits.GSWTRG = 1;
  IFS0bits.T4IF = 0;      counters ();   start_pid_reg = 1;                   //pid_control(MB_conf.CV_KP, MB_conf.CV_KD, MB_conf.CV_KI);
    }


void __ISR_AT_VECTOR (_UART5_RX_VECTOR, IPL4SRS) U5RXInterrupt(void)  {		IFS5bits.U5RXIF = 0;    
	usart5.mb_status.modb_receiving = 1;	    	
	while (U5STAbits.URXDA)	{ usart5.in_buffer[usart5.in_buffer_count++] = U5RXREG; }
	if (usart5.in_buffer_count>=IN_SIZE1) { usart5.mb_status.modb_received = 1; usart5.mb_status.modb_receiving = 0;}
...
void __ISR_AT_VECTOR (_UART5_TX_VECTOR, IPL4SRS) U5TXInterrupt(void)  {	
	...
	while ((!U5STAbits.UTXBF)&&(!usart5.mb_status.last_byte))		//copy if buff  isn't full and byte is not last
		...
		if (usart5.out_buffer_count >= (usart5.number_send))	{	usart5.mb_status.last_byte = 1;	IEC5bits.U5TXIE = 0;	}
		}
	IFS5bits.U5TXIF=0;
}
...
void __ISR_AT_VECTOR (_INPUT_CAPTURE_3_VECTOR, IPL4SRS) IC3Interrupt(void)   {IC3Interrupt_(); measure_sync (); IFS0bits.IC3IF = 0; }     //IC3Int3527 ();

void __ISR_AT_VECTOR (_INPUT_CAPTURE_4_VECTOR, IPL4SRS) IC4Interrupt(void)   {IC4Interrupt_(); measure_rpm (); IFS0bits.IC4IF = 0; }      //IC4Int3527 ();

void __ISR_AT_VECTOR (_INPUT_CAPTURE_7_VECTOR, IPL4SRS) IC7Interrupt(void)   {IC7_measure(); IC7Int (); IFS1bits.IC7IF = 0; }

void __ISR_AT_VECTOR (_INPUT_CAPTURE_8_VECTOR, IPL4SRS) IC8Interrupt(void)   {IC8_measure(); IC8Int (); IFS1bits.IC8IF = 0;}
